...
/**
 * Executes map-reduce job as one phase Each node will execute
 * map,shuffle&sort,reduce and write out the result
 * 
 * @author melhindi
 *
 */
public class SinglePhaseExecutor extends MapReduceExecutor {
	...
	public void submit(Job job) {
		...
		// tasks are added to the tasks member to be able to track progress
		...
		for (int i = 0; i < this.numMappers; i++) {
			Future<?> future = cluster
					.submit(new SinglePhaseTask(job.getInputs().get(i), job.getOutputs().get(i), i, this.nodeMap,
							job.getConfiguration().getPartitionColumn(), job.getMapperClass(), job.getReducerClass()));
			tasks.add(future);
		}
